...
public class PolicyController {
    ...
    public ResponseEntity<?> getPlans(@AuthenticationPrincipal User user) {
        ...
        return ResponseEntity.ok(List.of(
            Map.of("name", "BASIC",    "weeklyPremium", policyService.calculateDynamicPremium(user, Policy.PlanType.BASIC),
                   "coverage", 500,  "available", !highRisk,
                   "features", List.of("Rain protection", "Basic support")),
            Map.of("name", "STANDARD", "weeklyPremium", policyService.calculateDynamicPremium(user, Policy.PlanType.STANDARD),
                   "coverage", 1500, "available", !highRisk,
                   "features", List.of("Rain + Heat protection", "AQI alerts", "Priority support")),
            Map.of("name", "PREMIUM",  "weeklyPremium", policyService.calculateDynamicPremium(user, Policy.PlanType.PREMIUM),
                   "coverage", 3000, "available", true,
